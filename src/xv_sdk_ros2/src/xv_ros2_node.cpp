...
void xvision_ros2_node::initTopicAndServer(void)
{
    ...
    m_sgbm_raw_camera_publisher = image_transport::create_publisher(
        this, "xv_sdk/sgbmRaw/image");
    m_sgbm_raw_cameraInfo_pub = this->create_publisher<sensor_msgs::msg::CameraInfo>("xv_sdk/sgbmRaw/camera_info", 1);  
}
...
void xvision_ros2_node::publishSGBMRawImage(const sensor_msgs::msg::Image& image,const sensor_msgs::msg::CameraInfo& cameraInfo)
{
    m_sgbm_raw_camera_publisher.publish(image);//, cameraInfo);
    m_sgbm_raw_cameraInfo_pub->publish(cameraInfo);
}
